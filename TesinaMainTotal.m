...
TmA = [TmGen, T_shape(:,2:end)]; %Unisco i due campi di temperatura
TmB = flipud(TmA);
TmTot = [TmB ; TmA];

u1 = linspace(-0.03,0,5);
u2 = linspace(0,0.2,35);
u = [u1, u2(2:end)];
v = linspace(-L,L,10);

save('TmTot');

%Surface plot di tutto il sistema
figure(3)
surf(u,v,TmTot);
...
figure(6)
[co,ho] = contour(u,v,TmTot);
legend('T [°C]');
v = 30:0.5:150;
clabel(co,ho,v, 'LabelSpacing', 200, 'FontSize', 12);
%colormap hot
xlabel('x [m]','Fontsize',12);
ylabel('y [m]','Fontsize',12);
set(gca,'Fontsize',12);
...
figure(8)
plot(u,TmTot(ny,:), u, TmTot(1,:),'r--','LineWidth',2);
xlabel('x [m]','Fontsize',12)
...
%%Consistenza della griglia per y = 0
...
xlabel('Passo [m]','Fontsize',22)
ylabel('Potenza smaltita [W]','Fontsize',22)
hold on
grid on
set(gca,'FontSize',20)
%legend('y = W/2', 'y = 0','Soluzione Analitica')
